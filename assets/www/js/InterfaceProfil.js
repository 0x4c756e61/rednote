...
InterfaceProfil.prototype.construire = function () {
  ...
  var H = [];
  H.push('<button onclick="', this.nom, '.addProfil();" class="btn-floating waves-effect waves-light" style="position:fixed;bottom:10px;right:10px;" aria-label="', GTraductions.getValeur('ajouterUnProfil'),'"><i class="icon_plus_fin"></i></button>');
...
InterfaceProfil.prototype.validerProfilApresSuppCookies = function (aBypassCAS) {
  ...
  if (GApplication.etatConnexion) {
    if (this.profilCourant.serverUrl && this.profilCourant.serverUrl[this.profilCourant.serverUrl.length - 1] !== '/') {
                ...
                if (lUrl) {
                  ...
                  try {
                    ...
                    if (lJetonConnexion && lJetonConnexion.url && lJetonConnexion.login && lJetonConnexion.jeton) {
                      var lJetonConnexionSanitized = {};
                      lJetonConnexionSanitized.login = '' + lJetonConnexion.login;
                      lJetonConnexionSanitized.jeton = '' + lJetonConnexion.jeton;
                      lJetonConnexionSanitized.url = lThis.estUrlValide(lJetonConnexion.url);
                      $('#idInputURL').val(lJetonConnexionSanitized.url.href);
                      lThis.validerURL(lJetonConnexionSanitized.url, lJetonConnexionSanitized);
                    } else {
